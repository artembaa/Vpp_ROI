					...
					SI	   *ImageCoordY)
{
	...
	/* предварительное расположение изображени¤ - в геометрическом центре расширенного буфера */
	...
	{
		VPR_S_Left++;
		OutL = S_ITEM_SIZE - (VPR_R_Left - R_L); /* внешн¤¤ область изображени¤ - не покрыта матрицей */
	}
	else
		OutL = 0;
	...
	{
		VPR_S_Right--;
		OutR = S_ITEM_SIZE - (R_R - VPR_R_Right); /* внешн¤¤ область изображени¤ - не покрыта матрицей */
	}
	else
		OutR = 0;
	...
	{
		VPR_S_Top++;
		OutT = S_ITEM_SIZE - (VPR_R_Top - R_T); /* внешн¤¤ область изображени¤ - не покрыта матрицей */
	}
	else
		OutT = 0;
	...
	{
		VPR_S_Bottom--;
		OutB = S_ITEM_SIZE - (R_B - VPR_R_Bottom); /* внешн¤¤ область изображени¤ - не покрыта матрицей */
	}
	else
		OutB = 0;

	/* выравнивание позиции изображени¤ по отношению к ¤чейкам матрицы */
	...
	{
		VPR_S_Top--;  /* включение самого верхнего р¤да */
